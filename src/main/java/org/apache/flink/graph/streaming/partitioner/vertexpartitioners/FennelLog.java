...
public class FennelLog {
	...
	//	vertices.partitionCustom(new FennelPartitioner<>(new CustomKeySelectorLog<>(0), k, vertexCount, edgeCount), new CustomKeySelectorLog<>(0)).writeAsCsv(outputPath, FileSystem.WriteMode.OVERWRITE).setParallelism(k);
		vertices.partitionCustom(new FennelPartitioner<>(new CustomKeySelectorLog<>(0), k, vertexCount, edgeCount), (new CustomKeySelectorLog<>(0))).addSink(new TimestampingSinkv(outputPath)).setParallelism(k);
...
//	}
	...
	private static class FennelPartitioner<T> implements Serializable, Partitioner<T> {
		...
		public FennelPartitioner(CustomKeySelectorLog keySelector, int k, int n, int m) {
