...
import org.apache.flink.graph.streaming.partitioner.object.PdsGenerator;
...
public class Pds {
	...
	public static void main(String[] args) throws Exception {
		...
		edges.partitionCustom(new PdsPartitioner<>(new CustomKeySelector(0),k), new CustomKeySelector<>(0)).writeAsCsv(outputPath, FileSystem.WriteMode.OVERWRITE).setParallelism(k);
		//edges.partitionCustom(new PdsPartitioner<>(new CustomKeySelector(0),k), new CustomKeySelector<>(0))
	...
	private static class PdsPartitioner<T> implements Partitioner<T> {
		...
		public PdsPartitioner(CustomKeySelector keySelector, int k)
		{
		...
		private void make_pds_constraint() {
			...
			PdsGenerator pds_generator = new PdsGenerator();
		...
		private int initializeRowColPds() {
			int p = (int) Math.sqrt(k-1);
			if (!(p>0 && ((p*p+p+1) == k))){
				System.out.println("ERROR Num partitions "+k+" cannot be used for pds ingress.");
				System.exit(-1);
			}
			return p;
		}
