...
public class LinearGreedy {
	...
	public static void main(String[] args) throws Exception {
		...
		vertices.partitionCustom(new GreedyPartitioner<>(new CustomKeySelector<>(0), k, vertexCount), new CustomKeySelector<>(0)).writeAsCsv(outputPath, FileSystem.WriteMode.OVERWRITE).setParallelism(k);
		//vertices.partitionCustom(new GreedyPartitioner<>(new CustomKeySelector<>(0), k, vertexCount), (new CustomKeySelector<>(0))).addSink(new TimestampingSinkv(outputPath)).setParallelism(k);
	...
	private static class GreedyPartitioner<T> implements Serializable, Partitioner<T> {
		...
		public GreedyPartitioner(CustomKeySelector keySelector, int k, int n) {
